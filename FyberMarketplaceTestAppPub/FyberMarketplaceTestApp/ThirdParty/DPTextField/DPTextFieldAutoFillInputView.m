...
- (void)dismissSelfAnimated:(BOOL)animated {
    ...
    if (animated) {
        ...
        CGFloat centerY = [[self keyboardImageView] center].y;
        CGFloat toolbarHeight = [_textField toolbarHeight];
        UITextField *weakTextField = _textField;
        ...
        [UIView animateWithDuration:[_textField dismissAutoFillAnimationDuration] delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            ...
            [weakTextField setInputView:nil];
            [weakTextField reloadInputViews];
        }];
    } else {
...
- (void)buildTableView {
    ...
    NSInteger edgeInsetTop = -33;
    if ([[[UIDevice currentDevice] systemVersion] compare:@"7" options:NSNumericSearch] == NSOrderedAscending) {
        edgeInsetTop = -10;
    }
    [_tableView setContentInset:UIEdgeInsetsMake(edgeInsetTop, 0, 0, 0)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray *array = [_textField autoFillStrings];
    ...
    if (indexPath.row < array.count) {
        ...
        return cell; // Fix for a crash which involved dragging the table view cells in the sim. after a value was picked.
    }
}

- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath {
    NSArray *array = [_textField autoFillStrings];// Fix for a crash which involved dragging the table view cells in the sim. after a value was picked.
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ((UITableViewCellEditingStyleDelete == editingStyle) && (indexPath.row < array.count)) {
