...
#define kAuthorLabelHeight 30
#define kTimeLabelWidth 50

@implementation InneractiveFeed2TableCell
...
- (void)layoutSubviews {
    ...
    frame.origin.x = frame.size.width - kTimeLabelWidth - kInterfaceElementsIndent;
    frame.origin.y = kInterfaceElementsIndent;
    frame.size.width = kTimeLabelWidth;
    ...
    frame.origin.y = frame.size.height - kAuthorLabelHeight;
    frame.size.width -= frame.origin.x + kInterfaceElementsIndent;
    frame.size.height = kAuthorLabelHeight;
    ...
    float feedImageViewSize = self.contentView.bounds.size.height - (CGRectGetMaxY(self.avatarImageView.frame) + kInterfaceElementsIndent)
                                                                  - (self.authorNameLabel.frame.size.height + kInterfaceElementsIndent);
...
- (void)initInterface {
    ...
    _timeLabel.font = [UIFont fontWithName:@"Roboto-Light" size:18];
    _timeLabel.textColor = [UIColor colorWithRed:48/255.0 green:48/255.0 blue:48/255.0 alpha:1.0];
